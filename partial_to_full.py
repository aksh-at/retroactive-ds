...
class Partial_To_Full(Retroactive_DS):
    ...
    def get_prev_checkpoint(time):
        ret = 0

        while (ret + 1) < len(self.checkpoints) and time < self.checkpoints[ret+1][0]:
            ret += 1

        return ret

    def check_for_remake():
        self.m = len(self.ops)
    ...
    def Query(self, time, *args):
        target_checkpoint = get_prev_checkpoint(time)
        ...
        cur_time = self.ops.ceil_key(cur_time) # In case we deleted the op corresponding to checkpoint
    ...
    def Insert(self, time, op_name, *args):
        target_checkpoint = get_prev_checkpoint(time) + 1
